...
def extract_prose_existing_entities(
    ...
    V4.4.5 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v445(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v445(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v445(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v445(result, evidence)
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.4.5.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.4.5 and all AI details must be reviewed")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.4.5 PLAN")
    print("=" * 72)
