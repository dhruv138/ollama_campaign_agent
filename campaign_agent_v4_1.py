...
def creation_policy(entity: dict[str, Any]) -> tuple[str, str]:
    ...
    Return (status, reason), where status is:
      - "eligible": may be proposed/created
      - "review": detected, but never auto-created
      - "blocked": generic/low-specificity; do not create
    Existing-note matches bypass this policy because no new canon note is created.
    """
    ...
    generic_exact = {
        ...
        "the matron", "matron", "the head cleric", "head cleric",
        "the head priest", "head priest", "the guard", "guard",
        "the guards", "guards", "the sailor", "sailor",
        "the goliath sailor", "goliath sailor",
        "the sanitation workers", "sanitation workers",
    }
    ...
    # Titled but unnamed people/groups are useful mentions, but poor candidates
    # for automatic permanent notes.
    if etype == "npc" and key.startswith("the ") and not re.search(r"[A-Z]", mention[4:]):
        return "review", "descriptive NPC rather than a clear proper name"

    # New factions are especially easy for a small model to infer from ordinary
    # groups of people, so require manual review unless already matched.
    if etype == "faction":
        return "review", "new faction classification requires review"

    # Obvious class/type ambiguity: names ending in common equipment/place words
    # are still allowed, but held for review in --auto.
    ambiguous_terms = (" helm", " tattoo")
    if any(key.endswith(term) for term in ambiguous_terms):
...
- Keep the response compact: output only the JSON object and no commentary.
...
"""
    ...
    try:
        ...
        # Some small models occasionally wrap JSON in prose/code fences. Try
        # extracting the outer object once, but never silently "repair" broken JSON.
        ...
        try:
            ...
            raise ValueError(
                ...
                f"Parse failure: line {parse_error.lineno}, "
                f"column {parse_error.colno}, char {parse_error.pos}.\n"
