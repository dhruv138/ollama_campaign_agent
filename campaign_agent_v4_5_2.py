...
def extract_prose_existing_entities(
    ...
    V4.5.2 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v452(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v452(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v452(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v452(result, evidence)
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.5.2.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.5.2 and all AI details must be reviewed")
...
def build_change_plan(
    ...
    V4.5.2 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    print("\n" + "=" * 72)
    print("V4.5.2 SAFE CHANGE PLAN — READ ONLY")
    print("=" * 72)
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals are read-only in V4.5.2.")
    print("  ADD SESSION HISTORY is deterministic but remains proposal-only in this release.")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.5.2 PLAN")
    print("=" * 72)
