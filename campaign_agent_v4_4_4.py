...
def extract_prose_existing_entities(
    ...
    V4.4.4 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v444(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v444(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
    ...
    def classify(row: dict[str, Any]) -> str | None:
        ...
        rules = [
            ...
            ("two_tits_basement", ("two tits", "daggins' order", "daggins order", "fertility statue")),
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v444(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v444(result, evidence)
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.4.4.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.4.4 and all AI details must be reviewed")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.4.4 PLAN")
    print("=" * 72)
...
def _main_impl() -> int:
    ...
    # Existing note identity/type is authoritative. Collapse duplicate model/YAML/prose
    # candidates that resolve to the same vault note before computing conflicts.
    deduped_resolved: list[tuple[dict[str, Any], VaultNote | None, str]] = []
    seen_resolved: set[tuple[str, str]] = set()
    for entity, note, method in resolved_full:
        if note is not None:
            identity = ("note", str(note.path))
        else:
            identity = (
        ...
        deduped_resolved.append((entity, note, method))
    resolved_full = deduped_resolved
