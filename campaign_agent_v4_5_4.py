...
def extract_prose_existing_entities(
    ...
    V4.5.4 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v454(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v454(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v454(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v454(result, evidence)
...
def finish_transaction() -> Path | None:
    if not ACTIVE_TRANSACTION:
        return None
    _save_transaction_manifest()
...
def backup_file(path: Path, vault_root: Path, backup_dir: Path) -> Path:
    """
    V4.5.4: during a write transaction, preserve exactly one pre-run copy of
    each modified file under _backups/runs/<run-id>/before/<relative path>.
    Falls back to the legacy timestamp backup only outside a transaction.
    """
    rel = path.relative_to(vault_root)
    if ACTIVE_TRANSACTION:
        dest = ACTIVE_TRANSACTION["run_dir"] / "before" / rel
        if rel.as_posix() not in ACTIVE_TRANSACTION["modified"]:
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(path, dest)
            ACTIVE_TRANSACTION["modified"].add(rel.as_posix())
            _save_transaction_manifest()
        return dest
...
def rollback_run(vault_root: Path, backup_dir: Path, run_id: str) -> int:
    ...
    for rel in modified:
        print(f"  - {rel}")
    if created:
        print("Files created by the run that will be removed:")
        for rel in created:
            print(f"  - {rel}")

    if not ask_yes_no("Restore this entire run?", default=False):
    ...
    print(f"Rollback complete: {run_id}")
    return 0
...
def rollback_file(vault_root: Path, backup_dir: Path, rel_arg: str) -> int:
    ...
    for i, (run_id, _, manifest) in enumerate(matches, 1):
        print(f"  [{i}] before run {run_id}  source={manifest.get('source')}")
    print("  [c] cancel")
    ...
    run_id, manifest_path, _ = matches[int(answer) - 1]
...
def render_template(
    ...
    # V4.5.4: session provenance belongs in Session History, not YAML related:.
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.5.4; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.5.4 and all AI details must be reviewed")
...
def build_change_plan(
    ...
    V4.5.4 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.5.4; real writes are transaction-backed.")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.5.4 PLAN")
    print("=" * 72)
...
# ---------------------------------------------------------------------------
# V4.5.4 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def _main_impl() -> int:
    ...
    rollback_mode.add_argument("--rollback", metavar="RUN_ID", help="Restore every file changed by a V4.5.4 transaction.")
    rollback_mode.add_argument("--rollback-file", metavar="PATH", help="Restore one file from a selected V4.5.4 transaction backup.")
    ...
    # V4.5.4: do not add session links to YAML `related:`.
    ...
    manifest_path = finish_transaction()
