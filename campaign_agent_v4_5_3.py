...
def extract_prose_existing_entities(
    ...
    V4.5.3 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v453(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v453(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v453(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v453(result, evidence)
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.5.3; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.5.3 and all AI details must be reviewed")
...
def build_change_plan(
    ...
    V4.5.3 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.5.3.")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.5.3 PLAN")
    print("=" * 72)
...
# ---------------------------------------------------------------------------
# V4.5.3 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def _semantic_entry(proposal: dict[str, Any], session_link: str) -> str:
    ...
    AI interpretation remains visibly labeled; source excerpts are preserved.
    """
    ...
    for source in evidence:
        lines.append(f"  - Source: {_planner_short(source, 320)}")
    return "\n".join(lines)
