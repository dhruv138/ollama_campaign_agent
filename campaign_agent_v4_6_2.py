...
def extract_prose_existing_entities(
    ...
    V4.6.2 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v462(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v462(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v462(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v462(result, evidence)
...
def render_template(
    ...
    # V4.6.2: session provenance belongs in Session History, not YAML related:.
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.6.2; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.6.2 and all AI details must be reviewed")
...
def _planner_classify_relation(
    ...
    # V4.6.2: an entity that merely sees/notices/observes a clue is RELATED,
...
def _v462_evidence_overlap(row: dict[str, Any]) -> bool:
    """Require meaningful lexical overlap before calling evidence source-grounded."""
    evidence = " ".join(str(x) for x in (row.get("evidence") or []))
    if not evidence.strip():
        return False
    topic = f"{row.get('title') or ''} {row.get('detail') or ''}"
    stop = {
        "the","and","that","this","with","from","into","they","them","their","there",
    ...
    if not topic_tokens or not ev_tokens:
        return False
    overlap=topic_tokens & ev_tokens
    # At least two meaningful shared terms, or one unusually distinctive long term.
    return len(overlap) >= 2 or any(len(w) >= 9 for w in overlap)


def _v462_primary_home_score(note_title: str, note_type: str, section: str, row: dict[str, Any]) -> int:
    """Score which matched note should be the primary semantic home for a state row."""
    ...
    if key and key in title:
        score += 100
    elif key and key in detail:
        score += 35

    # Durable domain ownership beats an observer merely participating in the scene.
    nt=(note_type or "").lower()
    if section in {"developments","rumors","open_questions"} and nt == "faction":
        score += 25
    if section in {"clues","developments","open_questions"} and nt == "location":
        score += 18
    if section == "quests" and nt == "quest":
        score += 35
    if nt == "npc":
        padded=f" {title} "
...
def _v462_assign_primary_homes(
    updates: list[dict[str, Any]],
    campaign_state: dict[str, list[dict[str, Any]]],
) -> None:
    """Keep each semantic row on one best matched entity; demote duplicates to RELATED."""
    # Build identity from section/title/detail so equivalent rows can be compared.
    occurrences: dict[tuple[str,str,str], list[tuple[dict[str,Any],dict[str,Any]]]] = {}
    ...
    for ident, rows in occurrences.items():
        ...
        ranked=sorted(
            rows,
            key=lambda pair: _v462_primary_home_score(
                str(pair[0].get("entity") or ""),
                str(pair[0].get("type") or ""),
                section,
                pair[1],
            ),
            reverse=True,
        )
        winner_update,winner_prop=ranked[0]
        for update,proposal in ranked[1:]:
            ...
            if rel not in update["related"]:
                update["related"].append(rel)


def _v462_promote_quest_reviews(
    plan: dict[str, Any],
    campaign_state: dict[str, list[dict[str, Any]]],
    existing_notes: list[VaultNote],
    vault_root: Path,
    ...
    """Promote durable unresolved quest/objective state to REVIEW candidates."""
    existing_names={normalize_name(n.title) for n in existing_notes}
    already={
        (normalize_name(str(r.get("entity") or "")),str(r.get("type") or "").lower())
        for bucket in ("creates","reviews","ignores") for r in plan.get(bucket,[])
    }
    for row in campaign_state.get("quests",[]) or []:
        status=str(row.get("status") or "").lower()
        if not any(x in status for x in ("active","offered","unresolved")):
            continue
        ...
        if low.startswith("find missing sanitation workers") or low == "the missing sanitation workers":
            name="Missing Sanitation Workers"
        key=normalize_name(name)
        if key in existing_names or (key,"quest") in already:
            continue
...
def build_change_plan(
    ...
    V4.6.2 intentionally does not execute these campaign-state/entity-note
            ...
            if path_key not in seen_update_paths:
                ...
                for section, row in subjects:
                    ...
                    proposals.append({
                        ...
                        "classification": (
                            "source-grounded" if evidence and _v462_evidence_overlap(row)
    ...
    # V4.6.2 post-processing is read-only planning logic:
    # one semantic home per fact, evidence validation, and typed quest promotion.
    _v462_assign_primary_homes(plan["updates"], campaign_state)
    _v462_promote_quest_reviews(plan, campaign_state, existing_notes, vault_root, type_folders)
    return plan


def print_change_plan(plan: dict[str, Any]) -> None:
    print("\n" + "=" * 72)
    print("V4.6.2 SAFE CHANGE PLAN")
    print("=" * 72)
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.6.2; real writes are transaction-backed.")
    ...
    print("  V4.6.2 assigns one primary semantic home, validates source overlap, and promotes unresolved typed quests to REVIEW.")
    print("=" * 72)

def v462_canonical_candidate_name(entity: dict[str, Any]) -> str:
...
def v462_candidate_quality_gate(entity: dict[str, Any]) -> tuple[bool, str]:
    name = v462_canonical_candidate_name(entity)
...
def v462_apply_canonical_candidate_name(entity: dict[str, Any]) -> dict[str, Any]:
    canonical = v462_canonical_candidate_name(entity)
...
def v462_gate_candidate_rows(eligible, review, blocked):
    ...
    for rows, kept in ((eligible,kept_eligible),(review,kept_review)):
        for row in rows:
            entity=v462_apply_canonical_candidate_name(row[0])
            row=(entity,row[1],row[2],row[3])
            ok,reason=v462_candidate_quality_gate(entity)
            if ok:
                kept.append(row)
            else:
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.6.2 PLAN")
    print("=" * 72)
    ...
    eligible, review, blocked = v462_gate_candidate_rows(eligible, review, blocked)
    ...
    print("  - V4.6.2 adds candidate significance, conservative quest naming, observer/subject precision, and boundary-aware planner excerpts.")
...
# ---------------------------------------------------------------------------
# V4.6.2 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def select_new_entities(
    ...
    for entity, note, _ in resolved_full:
        ...
        entity = v462_apply_canonical_candidate_name(entity)
        status, reason = creation_policy(entity)
        quality_ok, quality_reason = v462_candidate_quality_gate(entity)
        if not quality_ok:
...
def _main_impl() -> int:
    ...
    rollback_mode.add_argument("--rollback", metavar="RUN_ID", help="Restore every file changed by a V4.6.2 transaction.")
    rollback_mode.add_argument("--rollback-file", metavar="PATH", help="Restore one file from a selected V4.6.2 transaction backup.")
    ...
    # V4.6.2: do not create empty transactions.
    ...
    # V4.6.2: the source session is not backed up or mutated merely because
    ...
    # V4.6.2: do not add session links to YAML `related:`.
