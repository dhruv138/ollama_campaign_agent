...
def extract_prose_existing_entities(
    ...
    V4.5.7 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v457(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v457(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v457(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v457(result, evidence)
...
def render_template(
    ...
    # V4.5.7: session provenance belongs in Session History, not YAML related:.
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.5.7; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.5.7 and all AI details must be reviewed")
...
def build_change_plan(
    ...
    V4.5.7 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.5.7; real writes are transaction-backed.")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.5.7 PLAN")
    print("=" * 72)
...
# ---------------------------------------------------------------------------
# V4.5.7 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def _main_impl() -> int:
    ...
    rollback_mode.add_argument("--rollback", metavar="RUN_ID", help="Restore every file changed by a V4.5.7 transaction.")
    rollback_mode.add_argument("--rollback-file", metavar="PATH", help="Restore one file from a selected V4.5.7 transaction backup.")
    ...
    # V4.5.7: do not create empty transactions.
    ...
    approved_creations = bool(selected_new)
    ...
    # V4.5.7: the source session is not backed up or mutated merely because
    ...
    # V4.5.7: do not add session links to YAML `related:`.
