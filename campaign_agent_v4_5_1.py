...
def extract_prose_existing_entities(
    ...
    V4.5.1 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v451(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v451(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v451(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v451(result, evidence)
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.5.1.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.5.1 and all AI details must be reviewed")
...
def _planner_classify_relation(
    ...
    # Conservative subject-at-start heuristic for descriptions like
    # "Deanira speaks...", "Riffle asks...", "Furgus sees...".
    first_words = " ".join(detail.split()[:8])
    if key in first_words:
        return "subject"
...
def _planner_entity_state(
    ...
    for section in ("quests", "clues", "developments", "rumors", "open_questions"):
        for row in campaign_state.get(section, []) or []:
            relation = _planner_classify_relation(entity_name, row)
            if relation == "subject":
...
def build_change_plan(
    ...
    V4.5.1 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    print("\n" + "=" * 72)
    print("V4.5.1 SAFE CHANGE PLAN — READ ONLY")
    print("=" * 72)
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals are read-only in V4.5.1.")
    print("  --dry-run changes no files.")
    print("  Only SUBJECT-classified campaign-state rows appear as UPDATE proposals; RELATED rows are context only. No planner UPDATE is executed by this release.")
    print("=" * 72)
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.5.1 PLAN")
    print("=" * 72)
