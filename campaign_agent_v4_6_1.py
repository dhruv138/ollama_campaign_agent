...
def extract_prose_existing_entities(
    ...
    V4.6.1 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v461(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v461(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v461(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v461(result, evidence)
...
def render_template(
    ...
    # V4.6.1: session provenance belongs in Session History, not YAML related:.
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.6.1; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.6.1 and all AI details must be reviewed")
...
def _planner_classify_relation(
    ...
    # V4.6.1: an entity that merely sees/notices/observes a clue is RELATED,
...
def build_change_plan(
    ...
    V4.6.1 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.6.1; real writes are transaction-backed.")
...
def v461_canonical_candidate_name(entity: dict[str, Any]) -> str:
...
def v461_candidate_quality_gate(entity: dict[str, Any]) -> tuple[bool, str]:
    name = v461_canonical_candidate_name(entity)
...
def v461_apply_canonical_candidate_name(entity: dict[str, Any]) -> dict[str, Any]:
    canonical = v461_canonical_candidate_name(entity)
...
def v461_gate_candidate_rows(eligible, review, blocked):
    ...
    for rows, kept in ((eligible,kept_eligible),(review,kept_review)):
        for row in rows:
            entity=v461_apply_canonical_candidate_name(row[0])
            row=(entity,row[1],row[2],row[3])
            ok,reason=v461_candidate_quality_gate(entity)
            if ok:
                kept.append(row)
            else:
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.6.1 PLAN")
    print("=" * 72)
    ...
    eligible, review, blocked = v461_gate_candidate_rows(eligible, review, blocked)
    ...
    print("  - V4.6.1 adds candidate significance, conservative quest naming, observer/subject precision, and boundary-aware planner excerpts.")
...
# ---------------------------------------------------------------------------
# V4.6.1 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def select_new_entities(
    ...
    for entity, note, _ in resolved_full:
        ...
        entity = v461_apply_canonical_candidate_name(entity)
        status, reason = creation_policy(entity)
        quality_ok, quality_reason = v461_candidate_quality_gate(entity)
        if not quality_ok:
...
def _main_impl() -> int:
    ...
    rollback_mode.add_argument("--rollback", metavar="RUN_ID", help="Restore every file changed by a V4.6.1 transaction.")
    rollback_mode.add_argument("--rollback-file", metavar="PATH", help="Restore one file from a selected V4.6.1 transaction backup.")
    ...
    # V4.6.1: do not create empty transactions.
    ...
    # V4.6.1: the source session is not backed up or mutated merely because
    ...
    # V4.6.1: do not add session links to YAML `related:`.
