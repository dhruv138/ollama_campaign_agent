...
""",
  ...
  "lore_entities": [
    ...
    combined_entities = extract_wikilink_entities(target, existing_notes, build_alias_index(existing_notes))
    ...
    preliminary = {
        ...
        "open_questions": extract_explicit_questions(target.body),
    }
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.4.1.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-only in V4.4.1 and all AI details must be reviewed")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.4.1 PLAN")
    print("=" * 72)
