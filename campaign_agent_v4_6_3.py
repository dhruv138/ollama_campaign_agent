...
def extract_prose_existing_entities(
    ...
    V4.6.3 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v463(body: str) -> list[dict[str, Any]]:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v463(canonical_body)
    preliminary = {
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.6.3; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.6.3 and all AI details must be reviewed")
...
def _planner_short(text: str, limit: int = 220) -> str:
    """Compact planner text while preferring sentence/clause boundaries."""
    text = re.sub(r"\s+", " ", str(text or "")).strip()
    if len(text) <= limit:
        return text
    cut = text[:limit - 3].rstrip()
    sentence_stop = max(cut.rfind(". "), cut.rfind("? "), cut.rfind("! "))
    if sentence_stop >= int(limit * 0.55):
        return cut[:sentence_stop + 1].strip()
    clause_stop = max(cut.rfind("; "), cut.rfind(": "))
    if clause_stop >= int(limit * 0.70):
        return cut[:clause_stop + 1].strip()
    return cut + "..."
...
def _planner_classify_relation(
    ...
    # V4.6.3: an entity that merely sees/notices/observes a clue is RELATED,
    # not the semantic subject of the observed evidence.
    section = str(row.get("_planner_section") or "")
    padded_title = f" {title} "
    observer_only = (
        section == "clues"
        and title.startswith(key + " ")
        and any(v in padded_title for v in (" sees ", " saw ", " notices ", " noticed ", " observes ", " observed "))
    )
    if observer_only:
        return "related"
...
def _v463_evidence_overlap(row: dict[str, Any]) -> bool:
    ...
    if not evidence.strip():
        return False
    topic = f"{row.get('title') or ''} {row.get('detail') or ''}"
    stop = {"the","and","that","this","with","from","into","they","them","their","there",
            "then","when","where","what","have","has","had","was","were","for","but","not",
            "his","her","she","him","about","after","before","through","one","two","some"}
    topic_tokens={w for w in normalize_name(topic).split() if len(w)>=4 and w not in stop}
    ev_tokens={w for w in normalize_name(evidence).split() if len(w)>=4 and w not in stop}
    overlap=topic_tokens & ev_tokens
    return bool(topic_tokens and ev_tokens and (len(overlap)>=2 or any(len(w)>=9 for w in overlap)))


def _v463_primary_home_score(note_title: str, note_type: str, section: str, row: dict[str, Any]) -> int:
    key=normalize_name(note_title)
    title=normalize_name(str(row.get("title") or ""))
    detail=normalize_name(str(row.get("detail") or ""))
    score=100 if key and key in title else (35 if key and key in detail else 0)
    nt=(note_type or "").lower()
    if section in {"developments","rumors","open_questions"} and nt=="faction": score+=25
    if section in {"clues","developments","open_questions"} and nt=="location": score+=18
    if section=="quests" and nt=="quest": score+=35
    if nt=="npc":
        padded=f" {title} "
        if title.startswith(key+" ") and any(v in padded for v in
            (" sees "," saw "," notices "," noticed "," observes "," observed ",
             " visits "," visited "," returns "," returned ")):
            score-=45
    return score


def _v463_assign_primary_homes(updates: list[dict[str, Any]], campaign_state: dict[str, list[dict[str, Any]]]) -> None:
    occurrences={}
    for update in updates:
        for proposal in update.get("proposals") or []:
            ident=(str(proposal.get("section") or ""),
                   normalize_name(str(proposal.get("title") or "")),
                   normalize_name(str(proposal.get("detail") or "")))
            occurrences.setdefault(ident,[]).append((update,proposal))
    for ident, rows in occurrences.items():
        if len(rows)<2: continue
        section=ident[0]
        ranked=sorted(rows,key=lambda pair:_v463_primary_home_score(
            str(pair[0].get("entity") or ""),str(pair[0].get("type") or ""),section,pair[1]),reverse=True)
        for update,proposal in ranked[1:]:
            update["proposals"].remove(proposal)
            rel={"section":section,"title":proposal.get("title") or "Campaign fact"}
            if rel not in update["related"]: update["related"].append(rel)


def _v463_quest_todo_overlap(quest_row: dict[str, Any], todos: list[dict[str, Any]]) -> bool:
    q=normalize_name(f"{quest_row.get('title') or ''} {quest_row.get('detail') or ''}")
    q_tokens={w for w in q.split() if len(w)>=4}
    for todo in todos or []:
        t=normalize_name(f"{todo.get('title') or ''} {todo.get('detail') or ''}")
        overlap=q_tokens & {w for w in t.split() if len(w)>=4}
        if len(overlap)>=2: return True
    return False


def _v463_candidate_key(name: str, etype: str) -> tuple[str,str]:
    low=normalize_name(name)
    if etype=="quest" and ("missing sanitation workers" in low or "find missing sanitation workers" in low):
        low="missing sanitation workers"
    return low,etype


def _v463_promote_quest_reviews(plan, campaign_state, existing_notes, vault_root, type_folders) -> None:
    """Promote only offered quests or objectives that overlap an explicit party TODO."""
    existing={normalize_name(n.title) for n in existing_notes}
    todos=campaign_state.get("todos",[]) or []
    for row in campaign_state.get("quests",[]) or []:
        status=str(row.get("status") or "").lower()
        offered="offered" in status
        todo_overlap=_v463_quest_todo_overlap(row,todos)
        if not (offered or todo_overlap): continue
        raw=str(row.get("title") or "").strip()
        if not raw: continue
        name="Missing Sanitation Workers" if (
            "missing sanitation workers" in normalize_name(raw) or
            "find missing sanitation workers" in normalize_name(raw)
        ) else raw
        if normalize_name(name) in existing: continue
        folder=type_folders.get("quest","")
        plan["reviews"].append({
            "entity":name,"mention":raw,"type":"quest","significance":"meaningful",
            "reason":"explicit objective overlaps a party TODO; requires human review" if todo_overlap
                     else "explicit offered quest; requires human review",
            "proposed_path":str(Path(folder)/f"{safe_filename(name)}.md") if folder else None,
            "write_enabled":False,"possible_duplicates":[],"provenance":"campaign-state-quest"})


def _v463_consolidate_candidates(plan: dict[str, Any]) -> None:
    """Merge equivalent CREATE/REVIEW candidates; typed quest provenance wins."""
    groups={}
    for bucket in ("creates","reviews"):
        for row in plan.get(bucket,[]) or []:
            key=_v463_candidate_key(str(row.get("entity") or row.get("mention") or ""),
                                    str(row.get("type") or "").lower().strip())
            groups.setdefault(key,[]).append((bucket,row))
    creates=[]; reviews=[]
    for key,rows in groups.items():
        bucket,chosen=sorted(rows,key=lambda br:(
            0 if br[1].get("provenance")=="campaign-state-quest" else 1,
            0 if br[0]=="reviews" else 1))[0]
        chosen=dict(chosen)
        if key==("missing sanitation workers","quest"):
            chosen["entity"]="Missing Sanitation Workers"
            chosen["type"]="quest"
            chosen["reason"]="explicit objective overlaps a party TODO; requires human review"
            bucket="reviews"
            p=Path(chosen.get("proposed_path") or "")
            if str(p.parent) not in {"","."}: chosen["proposed_path"]=str(p.parent/"Missing Sanitation Workers.md")
        dupes=[]; seen=set()
        for _,r in rows:
            for d in r.get("possible_duplicates") or []:
                dk=(d.get("path"),d.get("title"),d.get("type"))
                if dk not in seen: seen.add(dk); dupes.append(d)
        chosen["possible_duplicates"]=dupes
        (creates if bucket=="creates" else reviews).append(chosen)
    plan["creates"]=creates; plan["reviews"]=reviews
...
def build_change_plan(
    ...
    V4.6.3 intentionally does not execute these campaign-state/entity-note
            ...
            if path_key not in seen_update_paths:
                ...
                for section, row in subjects:
                    ...
                    proposals.append({
                        ...
                        "classification": (
                            "source-grounded" if evidence and _v463_evidence_overlap(row)
...
def print_change_plan(plan: dict[str, Any]) -> None:
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.6.3; real writes are transaction-backed.")
    ...
    print("  If no mutations are approved, no transaction/manifest is created.")
    print("  Only SUBJECT-classified rows are eligible for semantic UPDATE; RELATED rows remain context only.")
    print("  V4.6.3 consolidates equivalent candidates, requires durable quest evidence, and blocks category-like item collections.")
    print("=" * 72)

def v463_canonical_candidate_name(entity: dict[str, Any]) -> str:
    name = str(entity.get("name") or entity.get("canonical_name") or entity.get("mention") or "").strip()
    if str(entity.get("type") or "").lower() == "quest" and name.casefold() == "the missing sanitation workers":
        return "Missing Sanitation Workers"
    return name


def v463_candidate_quality_gate(entity: dict[str, Any]) -> tuple[bool, str]:
    name = v463_canonical_candidate_name(entity)
    etype = str(entity.get("type") or "").strip().lower()
    low = name.casefold()
    significance = str(entity.get("significance") or "").strip().casefold()
    if not name:
        return False, "empty candidate"
    if etype == "item" and any(x in low for x in ("phallic pin","vaginal staff","staff topped with","wooden knot","tattoo")):
        return False, "descriptive prop/clue rather than a durable standalone item"
    if etype == "item" and low in {"fertility statue", "fertility statues", "the fertility statues"}:
        return False, "category/collection objective rather than one durable standalone item"
    if etype == "npc":
        if low in {"sanitation worker","sanitation workers","searcher","searchers","sailor","sailors"}:
            return False, "unnamed group rather than a trackable NPC"
        if low in {"goliath sailor","the goliath sailor"} and significance != "recurring":
...
def v463_apply_canonical_candidate_name(entity: dict[str, Any]) -> dict[str, Any]:
    canonical = v463_canonical_candidate_name(entity)
    if canonical and canonical != str(entity.get("name") or ""):
        entity = dict(entity)
        entity["name"] = canonical
    return entity


def v463_gate_candidate_rows(eligible, review, blocked):
    kept_eligible, kept_review, new_blocked = [], [], list(blocked)
    seen={(normalize_name(r[0].get("name") or ""),str(r[0].get("type") or "").lower()) for r in new_blocked}
    for rows, kept in ((eligible,kept_eligible),(review,kept_review)):
        for row in rows:
            entity=v463_apply_canonical_candidate_name(row[0])
            row=(entity,row[1],row[2],row[3])
            ok,reason=v463_candidate_quality_gate(entity)
            if ok:
                kept.append(row)
            else:
                key=(normalize_name(entity.get("name") or ""),str(entity.get("type") or "").lower())
                if key not in seen:
                    new_blocked.append((entity,row[1],row[2],reason)); seen.add(key)
    return kept_eligible, kept_review, new_blocked
...
def print_plan(
    ...
    eligible, review, blocked = v463_gate_candidate_rows(eligible, review, blocked)
    ...
    print("  - V4.6.3 adds candidate significance, conservative quest naming, observer/subject precision, and boundary-aware planner excerpts.")
...
# ---------------------------------------------------------------------------
# V4.6.3 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def select_new_entities(
    ...
    for entity, note, _ in resolved_full:
        ...
        entity = v463_apply_canonical_candidate_name(entity)
        status, reason = creation_policy(entity)
        quality_ok, quality_reason = v463_candidate_quality_gate(entity)
        if not quality_ok:
            status, reason = "blocked", quality_reason
        key = (norm, etype)
...
def _main_impl() -> int:
    ...
    approved_creations = bool(approved_new)
    ...
    if not has_approved_mutation:
        print("\nNo changes approved. No transaction created; vault unchanged.")
        print(f"Target unchanged: {target.rel_path}")
        return

    transaction = begin_transaction(backup_dir, target.rel_path)

    # V4.6.3: the source session is not backed up or mutated merely because
    ...
    # V4.6.3: do not add session links to YAML `related:`.
