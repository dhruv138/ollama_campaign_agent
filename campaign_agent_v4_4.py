...
def extract_wikilink_entities(target: VaultNote, existing_notes: list[VaultNote]) -> list[dict[str, Any]]:
    """Resolve existing [[wikilinks]] directly from the vault."""
    by_title = {normalize_name(n.title): n for n in existing_notes}
    found: list[dict[str, Any]] = []
    seen: set[str] = set()

    pattern = re.compile(r"\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|([^\]]+))?\]\]")
    for match in pattern.finditer(target.body):
        target_name = match.group(1).strip()
        display = (match.group(2) or target_name).strip()
        key = normalize_name(target_name)
        if not key or key in seen:
            continue
        seen.add(key)

        note = by_title.get(key)
        if note is None:
            # An explicit link may point to a not-yet-created note. Preserve its
            # target name, but don't invent a type here.
            continue
        ...
        found.append({
            ...
            "reason": "existing wikilink in session note",
            "from_wikilink": True,
        })
    return found
...
def extract_source_evidence(body: str) -> list[dict[str, Any]]:
    """Preserve raw source passages for important campaign facts."""
    ...
    rules = [
        ...
        ("island", re.compile(r"unchartable island|unplottable island|island in the distance", re.I)),
        ("academy_robes", re.compile(r"\brobes?\b.*\b(?:red|purple)\b|\b(?:red|purple)\b.*\brobes?\b", re.I)),
        ("black_feathers", re.compile(r"Black Feathers", re.I)),
        ("two_tits_basement", re.compile(r"Two Tits Inn|basement|phallic pins", re.I)),
    ]
    rows, seen = [], set()
    for paragraph in paragraphs:
        for kind, pattern in rules:
            if pattern.search(paragraph):
                ...
                if key not in seen:
                    seen.add(key)
                    rows.append({"kind": kind, "source": paragraph})
    return rows
...
""",
  ...
  "lore_entities": [
    ...
    combined_entities = extract_wikilink_entities(target, existing_notes)
    ...
    preliminary = {
        ...
        "open_questions": clues.get("open_questions", []),
    }
    combined_entities.extend(promote_clue_people(preliminary))
    combined_entities.extend(entities_from_raw_evidence(target.body))
    ...
    result = {
        ...
        "rumors": clues.get("rumors", []),
        "open_questions": clues.get("open_questions", []),
        "todos": _extract_explicit_todos(target.body),
        "relationships": [],
    }
    return result
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.4.")
    print("Model-written details are interpretations of the session and must be reviewed")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.4 PLAN")
    print("=" * 72)
