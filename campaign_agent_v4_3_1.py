...
def _extract_explicit_todos(body: str) -> list[dict[str, Any]]:
    """Parse explicit Todo list bullets deterministically instead of asking the model."""
    ...
    for line in lines:
        ...
        if stripped and not stripped.startswith(("-", "*")):
            # End at the next non-bullet content after the todo section.
            break
    return todos
...
def extract_entities(
    ...
    entities = _focused_pass(
        ...
        """
Extract durable entity candidates:
...
""",
  ...
  "lore_entities": [
    ...
    combined_entities = list(entities.get("entities", []))
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.3.")
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.3.1 PLAN")
    print("=" * 72)
