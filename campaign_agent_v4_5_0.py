...
def extract_prose_existing_entities(
    ...
    V4.5.0 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v450(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v450(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v450(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v450(result, evidence)
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state and lore sections are report-only in V4.5.0.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.5.0 and all AI details must be reviewed")
...
def _planner_duplicate_candidates(
    ...
    def core(value: str) -> str:
        ...
        if words and words[0] == "the":
            words = words[1:]
        if words and words[-1] in {"inn", "tavern", "temple", "shrine"}:
            words = words[:-1]
        return " ".join(words)
    ...
    for note in existing_notes:
        ...
        c1, c2 = core(key), core(nk)
        if c1 and c2:
            score = max(score, SequenceMatcher(None, c1, c2).ratio())
        if score >= threshold:
...
def _planner_related_state(
    entity_name: str,
    campaign_state: dict[str, list[dict[str, Any]]],
) -> list[tuple[str, dict[str, Any]]]:
    """
    Conservative lexical association only. No model-generated relationships are
    introduced here. The output is a review proposal, never an automatic update.
    """
    key = normalize_name(entity_name)
    terms = {t for t in key.split() if len(t) >= 4}
    hits: list[tuple[str, dict[str, Any]]] = []

    for section in ("quests", "clues", "developments", "rumors", "open_questions"):
        for row in campaign_state.get(section, []) or []:
            hay = normalize_name(
                " ".join(str(row.get(k, "")) for k in ("title", "detail"))
            )
            if key and key in hay:
                hits.append((section, row))
                continue
            overlap = sum(1 for term in terms if term in hay)
            if len(terms) >= 2 and overlap >= 2:
                hits.append((section, row))
    return hits[:4]
...
def build_change_plan(
    ...
    V4.5.0 intentionally does not execute these campaign-state/entity-note
    ...
    for entity, note, method in resolved:
        ...
        if note is not None:
            ...
            if path_key not in seen_update_paths:
                ...
                proposals = []
                for section, row in _planner_related_state(note.title, campaign_state):
...
def print_change_plan(plan: dict[str, Any]) -> None:
    print("\n" + "=" * 72)
    print("V4.5.0 SAFE CHANGE PLAN — READ ONLY")
    print("=" * 72)
    ...
    for i, row in enumerate(updates, 1):
        ...
        if not proposals:
            print("       No campaign-state update confidently associated.")
        for proposal in proposals:
            print(f"       • {proposal.get('title')} [{proposal.get('classification')}]")
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals are read-only in V4.5.0.")
    print("  --dry-run changes no files.")
    print("  Campaign-state UPDATE proposals are NOT executed by this release.")
    print("=" * 72)
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.5.0 PLAN")
    print("=" * 72)
