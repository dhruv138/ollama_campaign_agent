...
def extract_prose_existing_entities(
    ...
    V4.6.0 resolves all candidate spans first, then keeps the longest match when
...
def extract_explicit_questions_v460(body: str) -> list[dict[str, Any]]:
    """
...
def attach_evidence_to_state_v460(result: dict[str, Any], evidence: list[dict[str, Any]]) -> None:
    """
...
""",
  ...
  "lore_entities": [
    ...
    source_questions = extract_explicit_questions_v460(canonical_body)
    preliminary = {
    ...
    attach_evidence_to_state_v460(result, evidence)
...
def render_template(
    ...
    # V4.6.0: session provenance belongs in Session History, not YAML related:.
...
def print_campaign_state(state: dict[str, list[dict[str, Any]]]) -> None:
    ...
    if not any_rows:
        print("  (none)")
    print("\nNOTE: Campaign-state extraction is report-first in V4.6.0; only individually approved SUBJECT proposals may be written.")
    print("Model-written details are interpretations; OPEN QUESTIONS are source-grounded in V4.6.0 and all AI details must be reviewed")
...
def build_change_plan(
    ...
    V4.6.0 intentionally does not execute these campaign-state/entity-note
...
def print_change_plan(plan: dict[str, Any]) -> None:
    ...
    print("\nWRITE STATUS")
    print("  Planner proposals require approval in V4.6.0; real writes are transaction-backed.")
...
def v460_candidate_quality_gate(entity: dict[str, Any]) -> tuple[bool, str]:
    """Conservative deterministic gate for NEW standalone-note candidates."""
    ...
    if etype == "item":
        detail_terms = (
            "phallic pin", "phallic pins", "vaginal staff",
            "staff topped with", "wooden knot", "tattoo",
        )
        if any(term in low for term in detail_terms):
            return False, "descriptive prop/clue rather than a durable standalone item"

    if etype == "npc" and low in {
        "sanitation worker", "sanitation workers", "searcher", "searchers",
        "sailor", "sailors",
    }:
...
def v460_gate_candidate_rows(eligible, review, blocked):
    """Move low-quality NEW candidates to BLOCKED; existing-note matches are untouched."""
    kept_eligible, kept_review, new_blocked = [], [], list(blocked)
    seen = {
        (normalize_name(r[0].get("name") or ""), str(r[0].get("type") or "").lower())
        for r in new_blocked
    }
    for rows, kept in ((eligible, kept_eligible), (review, kept_review)):
        for row in rows:
            entity = row[0]
            ok, reason = v460_candidate_quality_gate(entity)
            if ok:
                kept.append(row)
            else:
                key = (normalize_name(entity.get("name") or ""), str(entity.get("type") or "").lower())
                if key not in seen:
                    new_blocked.append((entity, row[1], row[2], reason))
                    seen.add(key)
...
def print_plan(
    ...
    print("\n" + "=" * 72)
    print("CAMPAIGN AGENT V4.6.0 PLAN")
    print("=" * 72)
    ...
    eligible, review, blocked = v460_gate_candidate_rows(eligible, review, blocked)
    ...
    print("  - V4.6 filters descriptive props, generic groups, and sentence fragments before CREATE/REVIEW.")
...
# ---------------------------------------------------------------------------
# V4.6.0 CONTROLLED APPROVAL + WRITE HELPERS
# ---------------------------------------------------------------------------
...
def select_new_entities(
    ...
    for entity, note, _ in resolved_full:
        ...
        status, reason = creation_policy(entity)
        quality_ok, quality_reason = v460_candidate_quality_gate(entity)
        if not quality_ok:
...
def _main_impl() -> int:
    ...
    rollback_mode.add_argument("--rollback", metavar="RUN_ID", help="Restore every file changed by a V4.6.0 transaction.")
    rollback_mode.add_argument("--rollback-file", metavar="PATH", help="Restore one file from a selected V4.6.0 transaction backup.")
    ...
    # V4.6.0: do not create empty transactions.
    ...
    # V4.6.0: the source session is not backed up or mutated merely because
    ...
    # V4.6.0: do not add session links to YAML `related:`.
