#!/usr/bin/env python3
"""
campaign_agent_v3.py
Conservative Obsidian campaign-note processor: Qwen extracts; Python validates and writes.

Features
--------
- Processes one Markdown note at a time.
- Scans the vault for existing NPC/location/faction/quest/item/session notes.
- Uses Ollama only to extract candidate entities from the target note.
...
Example
-------
    python3 campaign_agent_v3.py "Sessions/Session 38.md" --dry-run
    python3 campaign_agent_v3.py "Sessions/Session 38.md"
    python3 campaign_agent_v3.py "Sessions/Session 38.md" --auto
"""
...
def is_low_specificity_entity(entity: dict[str, Any]) -> bool:
    """Block automatic creation of generic descriptions/places."""
    ...
    generic_exact = {
        ...
        "the matron", "matron", "the head cleric", "the head priest",
        "the guard", "guard", "the guards", "guards",
    }
    if key in generic_exact:
        return True

    # "The X" can still be a proper name, so don't reject it generally.
    # Reject very short single-word common-looking candidates.
    words = key.split()
    if len(words) == 1 and len(key) < 5:
        return True
    return False
...
def extract_entities(
    ...
    # v3 deliberately does NOT send the vault catalog to Qwen. Python owns
    # existence/matching; the model only extracts candidates from this note.
    prompt = f"""
Extract named campaign entities explicitly mentioned in the D&D session note below.

Entity types:
- npc: named people/creatures or a clearly recurring titled individual
- location: specifically named places
- faction: specifically named organizations/groups
- quest: specifically named missions, investigations, or objectives
- item: specifically named/notable objects

Rules:
- Use only text actually supported by the note.
- Do not invent campaign facts or generic D&D lore.
- Do not decide whether an entity already exists in the vault.
- Do not output relationships.
- Prefer the exact text used in the note for "mention".
- "name" should be the best concise canonical form supported by the note.
- Avoid generic descriptions such as "the city", "the bar", "the priest",
  "the sewers", or ordinary unnamed objects unless the wording clearly acts
  as a proper name.
- Deduplicate repeated mentions of the same entity.
- If uncertain whether something is a campaign entity, omit it.
...
{{
  "entities": [
    {{
      ...
      "name": "best name supported by the note",
...
def render_template(
    ...
    if template_path and template_path.exists():
        ...
        body = (
            f"# {title}\n\n"
            "## Summary\n\n"
            "Created by campaign_agent_v3.py. Review and expand this note.\n\n"
            "## Party Knowledge\n\n"
            "## Session History\n\n"
...
def print_plan(
    ...
    for entity, note, method in resolved:
        ...
        elif is_low_specificity_entity(entity):
            blocked.append((entity, note, method))
        else:
            proposed.append((entity, note, method))
    ...
    if not proposed:
        print("  (none)")
    for entity, _, _ in proposed:
        action = "eligible for creation" if create_missing else "not created"
        print(f"  ? {entity.get('mention')!r} -> {entity.get('name')} "
              f"[{entity.get('type')}, {action}]")

    print("\nBLOCKED / LOW-SPECIFICITY")
    if not blocked:
        print("  (none)")
    for entity, _, _ in blocked:
        print(f"  ! {entity.get('mention')!r} [{entity.get('type')}] "
              f"- will not create a note automatically")
    ...
    print("  - Add wikilinks only for matched or approved/created entities.")
    print("  - Update target YAML only with matched or created entity notes.")
    if create_missing:
        print("  - Create only specific, non-conflicting missing entities.")
    print("  - Do NOT write model-generated semantic relationships.")
    print("  - Add the session to related: on matched/created entity notes when enabled.")
...
def main() -> int:
    ...
    for e in result.get("entities", []):
        ...
        if not mention or not name:
            continue
        e = {"mention": mention, "name": name, "type": etype}
    ...
    for entity, note, method in resolved_full:
        ...
        if entity_key in conflict_keys or is_low_specificity_entity(entity):
